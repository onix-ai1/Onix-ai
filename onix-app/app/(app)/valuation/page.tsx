...
function DebtEngineCalculator() {
  ...
  const inp: React.CSSProperties = { background: 'var(--onix-surface)', border: '1px solid var(--onix-border)', color: 'var(--onix-text)', borderRadius: 8, padding: '8px 12px', fontSize: 13, width: '100%', outline: 'none' };
  const lbl: React.CSSProperties = { fontSize: 11, color: 'var(--onix-muted)', marginBottom: 4, display: 'block' };

  return (
    <div className="rounded-xl p-5 mt-6" style={{ background: 'var(--onix-card)', border: '1px solid var(--onix-border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(201,168,76,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--onix-gold)" strokeWidth={2}>
            <rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/><path d="M7 8h.01M11 8h5M7 12h.01M11 12h5"/>
          </svg>
        </div>
        <div>
          <h2 style={{ fontSize: 14, fontWeight: 600, color: 'var(--onix-text)', margin: 0 }}>Debt Engine Calculator</h2>
          <p style={{ fontSize: 11, color: 'var(--onix-muted)', margin: 0 }}>Model acquisition debt structure & serviceability</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div><label style={lbl}>Deal / Acquisition Value ($)</label><input type="number" value={dealValue} onChange={e => setDealValue(e.target.value)} style={inp} placeholder="10000000" /></div>
          <div><label style={lbl}>Equity Contribution (%)</label><input type="number" value={equityPct} onChange={e => setEquityPct(e.target.value)} style={inp} placeholder="30" min="0" max="100" /></div>
          <div><label style={lbl}>Interest Rate (% p.a.)</label><input type="number" value={interestRate} onChange={e => setInterestRate(e.target.value)} style={inp} placeholder="8.5" step="0.1" /></div>
          <div><label style={lbl}>Loan Term (Years)</label><input type="number" value={termYears} onChange={e => setTermYears(e.target.value)} style={inp} placeholder="5" min="1" max="30" /></div>
          <div><label style={lbl}>Target EBITDA ($)</label><input type="number" value={ebitda} onChange={e => setEbitda(e.target.value)} style={inp} placeholder="2000000" /></div>
          <div style={{ marginTop: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 11, color: 'var(--onix-muted)' }}>Loan-to-Value (LTV)</span>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--onix-gold)' }}>{results.ltv.toFixed(1)}%</span>
            </div>
            <div style={{ height: 6, borderRadius: 4, background: 'var(--onix-border)' }}>
              <div style={{ height: '100%', borderRadius: 4, width: `${Math.min(results.ltv, 100)}%`, background: results.ltv > 80 ? 'var(--onix-red)' : results.ltv > 65 ? 'var(--onix-amber)' : 'var(--onix-gold)', transition: 'width 0.3s ease' }} />
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ borderRadius: 12, padding: '16px 20px', background: 'var(--onix-surface)', border: `1px solid ${dscrColor}33`, textAlign: 'center', marginBottom: 4 }}>
            <p style={{ fontSize: 11, color: 'var(--onix-muted)', margin: '0 0 4px' }}>Debt Service Coverage Ratio</p>
            <p style={{ fontSize: 36, fontWeight: 700, color: dscrColor, margin: 0, lineHeight: 1 }}>{results.dscr.toFixed(2)}x</p>
            <span style={{ fontSize: 11, fontWeight: 600, color: dscrColor, background: `${dscrColor}20`, borderRadius: 20, padding: '2px 10px', display: 'inline-block', marginTop: 8 }}>{dscrLabel}</span>
            <p style={{ fontSize: 10, color: 'var(--onix-muted)', margin: '8px 0 0' }}>Lenders typically require ≥ 1.25x</p>
          </div>
          {[
            ...
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderRadius: 8, background: 'var(--onix-surface)' }}>
              <span style={{ fontSize: 12, color: 'var(--onix-muted)' }}>{label}</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--onix-text)' }}>{value}</span>
            </div>
          ))}
        </div>
      </div>
